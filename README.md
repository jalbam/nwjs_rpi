...
7. If all works well, you can **distribute your project**. You will need these files at least (in the same folder): _libffmpegsumo.so_, _nw.pak_ and _Your_new_binary_file_ (or _nw_ and _package.nw_ instead). I would recommend including _fix_libudev.so.0_ and _fix_libudev.so.1_ optionally (modified as explained in _step 3_) if you think others could need them.
